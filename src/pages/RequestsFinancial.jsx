...
import { financialRequests as seedRequests } from '../data/mockData';
...
export default function RequestsFinancial() {
  ...
  const updateStatus = (id, status, amountApproved) => {
    setRequests((rs) => rs.map((r) => (r.id === id ? { ...r, status, amountApproved: amountApproved ?? r.amountApproved } : r)));
    setSelected(null);
  };

  const handleApprove = (r) => {
    updateStatus(r.id, 'Approved', r.amountRequested);
  ...
  const handleRelease = (r) => {
    updateStatus(r.id, 'Assistance Released');
