...
export default function Dashboard({ navigate }) {
  ...
  if (!data) return <div className="page-stack">Loading…</div>;

  const {
    stats: dashboardStats,
    weeklyReportsOverview,
    requestsByStatus,
    topRequestTypes,
            ...
            <ul className="donut-legend">
              {requestsByStatus.map((s) => (
                <li key={s.name}>
                  ...
                  <b>{s.value} ({total ? Math.round((s.value / total) * 100) : 0}%)</b>
                </li>
              ))}
            </ul>
          </div>
      ...
      <div className="grid-2fr-1fr">
        <div className="panel">
          ...
          <div className="announcement-strip">
            <p>{announcement?.title ?? 'No announcements yet.'}</p>
            {announcement && (
              <span>Posted on {announcement.date} &middot; {announcement.postedBy}</span>
            )}
          </div>
        </div>
        ...
        <div className="panel">
          ...
          <p style={{ color: '#6B7280', fontSize: 13 }}>Coming soon — no backend endpoint yet.</p>
        </div>
      </div>
    </div>
  );
